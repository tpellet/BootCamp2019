...
model; 
	...
	(c - b * c(-1)) ^ (-1 / eeis) - bbeta * b* (c(+1) - b * c) ^ (-1 / eeis)				=  bbeta * ((c(+1) - b * c) ^ (-1 / eeis)) * (1 - ddelta + r(+1));
	
	// 2. First order equation for labor-leisure
	w * ((c - b * c(-1)) ^ (-1 / eeis) - bbeta * b* (c(+1) - b * c) ^ (-1 / eeis))			= cchi * (l ^ (1 / eeta));
...
steady_state_model;
	...
	z		= 0;
	w		= (1 - aalpha) * y / l;
	cchi	=  w * (((1 - b) * c) ^ (-1 / eeis) - bbeta * b * ((1 - b) * c) ^ (-1 / eeis)) / (l ^ (1 / eeta));
